    ...
    for (const file of fieldFiles) {
      if (!PARCEL_IMAGE_ALLOWED_TYPES.has(file.type)) {
        return {
          ...
          fieldErrors: createFieldErrors(fieldName, "Only JPG, PNG, or WebP files are allowed."),
        };
      }
...
export function validateParcelImageAppendLimits(input: {
  ...
  const nextCounts: Record<ParcelImageFieldName, number> = {
    ...
    paymentSlipImages: input.currentPaymentSlipImageCount + input.files.paymentSlipImages.length,
  };
