...
function writeTableHeader(doc: PDFKit.PDFDocument, fonts: InvoiceFonts, y: number) {
    ...
    doc.text("Type", 64, y, { width: 96 });
    doc.text("Direction", 160, y, { width: 110 });
    doc.text("Parcel", 270, y, { width: 88 });
    doc.text("Recipient", 358, y, { width: 96 });
    doc.text("Amount", 454, y, { width: 101, align: "right" });
    doc.moveTo(40, y + 14)
        .lineTo(555, y + 14)
        .strokeColor("#dddddd")
        .stroke();
...
function writeTotals(
    ...
    const rows = [
        ["Credits", formatMmk(settlement.totals.creditsTotal)],
        ["Debits", formatMmk(settlement.totals.debitsTotal)],
        ["Net", formatMmk(settlement.totals.netTotal)],
    ] as const;

    let currentY = y;
    for (const [label, value] of rows) {
        doc.font(label === "Net" ? fonts.bold : fonts.regular)
            .fontSize(label === "Net" ? 11 : 9)
...
export async function renderMerchantSettlementInvoicePdf(
    ...
    return new Promise((resolve, reject) => {
        ...
        doc.font(fonts.bold).fontSize(18).text("Merchant Settlement", 40, 40);
        doc.font(fonts.regular).fontSize(9).text(`Settlement ${settlement.id}`, 40, 66);
        doc.font(fonts.bold).fontSize(14).text(formatMmk(settlement.totals.netTotal), 375, 40, {
            align: "right",
            width: 180,
        });
        ...
        writeKeyValue(doc, fonts, "Direction", settlement.type, 40, 140);
        writeKeyValue(doc, fonts, "Bank", settlement.snapshotBankName ?? "-", 310, 140);
        writeKeyValue(
            doc,
            fonts,
            "Bank Account",
            settlement.snapshotBankAccountNumber ?? "-",
            310,
            184,
        );
        writeKeyValue(doc, fonts, "Created By", settlement.createdByName, 40, 184);
        writeKeyValue(doc, fonts, "Confirmed By", settlement.confirmedByName ?? "-", 40, 228);
        writeKeyValue(doc, fonts, "Created", formatDate(settlement.createdAt), 310, 228);
        writeKeyValue(doc, fonts, "Updated", formatDate(settlement.updatedAt), 40, 272);
        writeKeyValue(
            doc,
            fonts,
            "Payment Proof",
        ...
        settlement.items.forEach((item, index) => {
            ...
            doc.text(fitText(item.candidateKind, 18), 64, y, { width: 96 });
            doc.text(fitText(item.direction, 22), 160, y, { width: 110 });
            doc.text(fitText(item.parcelCode, 16), 270, y, { width: 88 });
            doc.text(fitText(item.recipientName, 20), 358, y, { width: 96 });
            doc.text(formatMmk(item.snapshotAmount), 454, y, { width: 101, align: "right" });
            doc.moveTo(40, y + 14)
                .lineTo(555, y + 14)
                .stroke();
            y += 20;
